const { getAvailableEnvironments, getDefaultEnvironment } = require("../config/environments");
...
function prepareAndroidLocalFiles() {
  ...
  if (process.env.ANDROID_B64_FASTFILE_ENV_STAGING) {
    copyBase64File(process.env.ANDROID_B64_FASTFILE_ENV_STAGING, "android/fastlane/.env.staging");
  }
...
function prepareIosLocalFiles() {
  if (process.env.IOS_B64_FASTLANE_ENV_DEFAULT) {
    copyBase64File(process.env.IOS_B64_FASTLANE_ENV_DEFAULT, "ios/fastlane/.env.default");
  }
  if (process.env.IOS_B64_FASTLANE_ENV_STAGING) {
    copyBase64File(process.env.IOS_B64_FASTLANE_ENV_STAGING, "ios/fastlane/.env.staging");
  }
  if (process.env.IOS_B64_FASTLANE_ENV_PRODUCTION) {
    copyBase64File(process.env.IOS_B64_FASTLANE_ENV_PRODUCTION, "ios/fastlane/.env.production");
  }
...
function prepareReactNativeLocalFiles() {
  if (process.env.RN_B64_ENV_STAGING) {
    copyBase64File(process.env.RN_B64_ENV_STAGING, "config/environments/.env.staging.local");
  }
  if (process.env.RN_B64_ENV_PRODUCTION) {
    copyBase64File(process.env.RN_B64_ENV_PRODUCTION, "config/environments/.env.production.local");
  }
}
