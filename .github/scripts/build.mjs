...
async function actionHandler (options) {
    if (!options.platform) {
        console.error('Platform is not specified !')
        process.exit(9)
    }

    const expoCommandBase = `npx eas build --platform ${options.platform} --no-wait`
    const expoUpdateCommandBase = `npx eas update --auto`
    ...
    switch (buildOrUpdate) {
        ...
        case 'build':
            if (refType === 'branch' && branchName.startsWith('rc/') || refType === 'tag' && branchName.startsWith('v')) {
                ...
                await aExec(`${expoCommandBase} --profile=production`)
                process.exit(0)
            } else {
...
program
    ...
    .option('-p, --platform platform to build: ios, android', 'Target version')
