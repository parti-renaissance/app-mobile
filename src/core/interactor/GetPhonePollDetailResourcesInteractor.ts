...
export class GetPhonePollDetailResourcesInteractor {
  ...
  public async execute(campaignId: string, sessionId: string): Promise<PhonePollDetailResources> {
    const poll = await this.phoningCampaignRepository.getPhoningCampaignPoll(campaignId);
    const configuration =
      await this.phoningCampaignRepository.getPhoningSessionConfiguration(sessionId);
    return {
      poll,
      configuration,
    };
