...
export class GetTimelineFeedInteractor {
  ...
  public async execute(page: number): Promise<PaginatedResult<Array<TimelineFeedItem>>> {
    const zipCode = await this.profileRepository.getZipCode();
    const response = await this.timelineFeedRepository.getTimelineFeed(page, zipCode);
    return response;
  }
}
