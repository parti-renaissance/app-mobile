...
export const RestSignUpRequestMapper = {
  map: (data: SignUpFormData): RestSignUpRequest => {
    return {
      ...
      phone: PhoneNumberMapper.mapToString(data.phoneNumber, data.phoneCountryCode),
      address: {
...
export const PhoneNumberMapper = {
  mapToString: (phoneNumber: string, countryCode: string): string => {
    if (phoneNumber.startsWith("0")) {
      ...
      const callingCode = CountryRepository.getInstance().getCallingCodeForCountryCode(countryCode);
