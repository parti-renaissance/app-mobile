...
export class CountryRepository {
  ...
  public getCountries(): Country[] {
    ...
    const countries = (require("country-data").countries.all as CountryDataEntity[])
      .filter((country) => {
        return country.status === "assigned" && country.countryCallingCodes.length > 0;
      })
      .map((country): Country => {
        return {
          code: country.alpha2,
  ...
  public getCallingCodeForCountryCode(countryCode: string): string {
    return this.getCountries().find((c) => c.code === countryCode)?.callingCode ?? "+33";
  }
