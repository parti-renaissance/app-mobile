...
class EventRepository {
  ...
  public async getEvents(
    ...
    const restEvents = await this.apiService.getEvents(zipCode, page, eventFilters);
  ...
  public async getMainEvents(
    ...
    const restEvents = await this.apiService.getEvents(zipCode, 1, eventFilters, true);
    const paginationInfo = RestMetadataMapper.map(restEvents.metadata);
    return {
      paginationInfo: paginationInfo,
      result: restEvents.items.map(EventMapper.mapShortEvent),
