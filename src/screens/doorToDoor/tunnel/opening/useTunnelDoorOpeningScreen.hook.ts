...
export const useTunnelDoorOpeningScreen = (
  ...
  const [statefulState, setStatefulState] = useState<ViewState<DoorToDoorPollConfigDoorStatus[]>>(
    ViewState.Loading(),
  );
  const [isSendingChoice, setIsSendingChoice] = useState(false);
  const navigation =
    useNavigation<DoorToDoorTunnelModalNavigatorScreenProps<"TunnelDoorOpening">["navigation"]>();
