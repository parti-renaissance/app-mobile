...
export const useEventListScreen = (
  ...
  const [statefulState, setStatefulState] = useState<ViewState<PaginatedResult<Array<ShortEvent>>>>(
    ViewState.Loading(),
  );
  const navigation = useNavigation<EventNavigatorScreenProps<"Events">["navigation"]>();
