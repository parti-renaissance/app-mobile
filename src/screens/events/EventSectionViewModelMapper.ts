...
export const EventSectionViewModelMapper = {
  map: (events: Array<ShortEvent>, filter: EventFilter): Array<EventSectionViewModel> => {
    if (filter === "home") {
...
function mapCalendar(events: ShortEvent[]): EventSectionViewModel[] {
  ...
  for (const key of keys) {
    ...
    const eventsViewModel: Array<EventRowContainerViewModel> = eventsOfSection.map((event) => {
      return {
        type: "event",
...
function mapHome(events: ShortEvent[]): EventSectionViewModel[] {
  ...
  for (const key of keys) {
    ...
    const eventViewModels: Array<EventRowViewModel> = eventsOfSection.map((event) => {
...
function mapSection(key: string, firstEvent: ShortEvent): EventSectionRowViewModel {
  const now = new Date();
  const isToday = key === extractSectionKey(now);
  const name = isToday
    ? i18n.t("events.section_date_today")
    : DateFormatter.format(firstEvent.dateStart, i18n.t("events.section_date_format"));
  return {
    sectionName: name,
  };
}
