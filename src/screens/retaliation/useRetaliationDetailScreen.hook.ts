...
export const useRetaliationDetailScreen = (
  ...
  const [statefulState, setStatefulState] = useState<ViewState<Retaliation>>(ViewState.Loading());
  ...
  useEffect(() => {
    const fetchRetaliation = () => {
      RetaliationRepository.getInstance()
        ...
        .catch((error) => setStatefulState(ViewStateUtils.networkError(error, fetchRetaliation)));
    };
    fetchRetaliation();
  }, [retaliationId]);
  ...
  return {
    statefulState: ViewState.map(statefulState, RetaliationDetailViewModelMapper.map),
    onRetaliate,
  };
};
