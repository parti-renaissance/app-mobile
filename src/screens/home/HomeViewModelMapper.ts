...
function appendEvent(event: ShortEvent | undefined, rows: HomeSectionViewModel[]) {
  if (!event) {
    return;
  }
  rows.push({
...
function appendQuickPoll(quickPoll: StatefulQuickPoll | undefined, rows: HomeSectionViewModel[]) {
    ...
    data: [
      {
        ...
        value: {
          ...
          leadingAnswerViewModel: {
            ...
            formattedPercentage: NumberFormatter.formatPercent(leadingAnswer.votesPercentage / 100),
          ...
          totalVotes: i18n.t("home.quick_poll.votes_count", {
            ...
            format: NumberFormatter.formatDecimal(quickPoll.result.totalVotesCount),
          }),
        },
      },
    ],
...
function appendRegion(region: Region | undefined, rows: HomeSectionViewModel[]) {
...
function appendTimelineFeedError(state: ViewStateError, rows: HomeSectionViewModel[]) {
  rows.push({
    id: "feed",
    sectionViewModel: {
    ...
    switch (item.type) {
      ...
      case "retaliation":
        return {
          ...
          value: HomeRetaliationCardViewModelFromTimelineRetaliationMapper.map(item.value),
        };
      case "phoning":
        return {
          type: "feedPhoningCampaign",
          value: HomeFeedActionCampaignCardViewModelFromTimelineItemMapper.map(item.value),
        };
      case "doorToDoor":
        return {
