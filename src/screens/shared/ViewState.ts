...
export type ViewState<T> = ViewStateContent<T> | ViewStateError | ViewStateLoading;

const Error = <T>(errorMessage: string, onRetry: (() => void) | undefined): ViewState<T> => {
...
const map = <T, U>(viewState: ViewState<T>, transform: (input: T) => U): ViewState<U> => {
...
const merge = <T, U>(lhs: ViewState<T>, rhs: ViewState<U>): ViewState<{ lhs: T; rhs: U }> => {
