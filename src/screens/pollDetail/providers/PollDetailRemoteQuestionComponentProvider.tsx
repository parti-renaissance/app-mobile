  ...
  implements PollDetailComponentProvider<PollRemoteQuestionResult>
{
  ...
  private getComponent(question: Question, answer: Answer | undefined): JSX.Element {
    switch (question.type) {
      ...
      case "simple_field":
        return this.getTextComponent(question, (answer?.answer as TextAnswer) ?? { value: "" });
    }
  }

  private getTextComponent(question: Question, answer: TextAnswer): JSX.Element {
    const viewModel = PollDetailQuestionInputViewModelMapper.map(question, answer);
    return (
  ...
  private getSingleChoiceComponent(question: Question, answer: SingleChoiceAnswer): JSX.Element {
    const viewModel = PollDetailQuestionSingleChoiceViewModelMapper.map(question, answer);
    return (
  ...
  private getMultipleChoicesComponent(
    ...
    const viewModel = PollDetailQuestionMultipleChoicesViewModelMapper.map(question, answer);
    return (
      <PollDetailQuestionChoice
        viewModel={viewModel}
        toggleChoice={(choiceId) => {
          ...
          if (answer.choiceIds.includes(parseInt(choiceId, 10))) {
            choiceIds = answer.choiceIds.filter((id) => id !== parseInt(choiceId, 10));
          } else {
