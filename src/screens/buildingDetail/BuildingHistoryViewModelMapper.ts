...
export const BuildingHistoryViewModelMapper = {
  map: (history: BuildingHistoryPoint[]): BuildingHistoryViewModel => {
    ...
    buildingsMap.forEach((buildingHistory, buildingName) => {
      ...
      buildingsViewModel.push(buildingVisitsHistoryVieModel(buildingName, dateRecords));
    });
    return {
...
function footnote(history: BuildingHistoryPoint[]): string {
  ...
  if (questioners.length > 1) {
    ...
    const concatenated =
      beforeLast + i18n.t("building.history.questionersListLastSeparator") + last;
    questioners.push(concatenated);
  }
  return i18n.t("building.history.questionersFormat", {
    questioners: questioners.join(i18n.t("building.history.questionersListSeparator")),
  });
}

function doorVisitViewModel(historyPoint: BuildingHistoryPoint): KeyValueCellViewModel {
  return {
...
function groupByBuilding(history: BuildingHistoryPoint[]): Map<string, BuildingHistoryPoint[]> {
...
function groupByDate(history: BuildingHistoryPoint[]): Map<string, BuildingHistoryPoint[]> {
  var buildingMap = new Map<string, BuildingHistoryPoint[]>();
  history.forEach((historyPoint) => {
