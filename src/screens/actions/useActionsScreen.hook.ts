...
export const useActionsScreen = (): {
  ...
  const [statefulState, setStatefulState] = useState<ViewState<ReadonlyArray<ActionRowViewModel>>>(
    ViewState.Loading(),
  );
  ...
  const navigation = useNavigation<ActionsNavigatorScreenProps<"Actions">["navigation"]>();
