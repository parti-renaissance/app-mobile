...
export const PhonePollSatisfactionViewModelMapper = {
  map: (
    ...
    const sections: Array<PhonePollSatisfactionSectionViewModel> = questions.map((question) => {
      const answer = answers.get(question.code);
      switch (question.type) {
        case "boolean":
          const isYesAnswer = answer?.type === "boolean" ? answer?.value : undefined;
          return {
            id: question.code,
        ...
        case "note":
          const rate = answer?.type === "rate" ? (answer?.value as number) ?? 0 : 0;
          return {
            id: question.code,
            title: question.label,
            data: [
