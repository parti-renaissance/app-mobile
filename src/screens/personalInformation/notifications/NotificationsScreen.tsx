...
import { SectionList, SectionListRenderItemInfo, StyleSheet, Text } from "react-native";
...
const NotificationsContent = (
  ...
  const onNotificationChanged = (id: string, isSelected: boolean) => {
    if (id === ID_PUSH) {
      ...
      if (isSelected && !content.notificationsEnabled.includes(id)) {
        ...
        newSubscriptions = content.notificationsEnabled.filter((subscription) => {
  ...
  const renderItem = ({ item }: SectionListRenderItemInfo<NotificationRowViewModel>) => {
    return <NotificationRowView viewModel={item} onSelectionChanged={onNotificationChanged} />;
  };
  return (
    <>
...
const NotificationsScreen = (props: NotificationsScreenProps) => {
  const [statefulState, setStatefulState] = useState<ViewState<GetNotificationsInteractorResult>>(
    ViewState.Loading(),
  );
