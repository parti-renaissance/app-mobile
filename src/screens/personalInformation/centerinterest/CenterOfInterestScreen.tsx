...
const CenterOfInterestContent = (
  ...
  const [viewModel, setViewModel] = useState(CentersOfInterestViewModelMapper.map(content));
  ...
  return (
    <View style={styles.contentContainer}>
      ...
      <FlatList
        ...
        numColumns={3}
        ListHeaderComponent={() => {
          return <Text style={styles.title}>{i18n.t("centerofinterest.description")}</Text>;
        }}
      />
      <View style={styles.bottomContainer}>
        <PrimaryButton title={i18n.t("centerofinterest.save")} onPress={submit} />
      </View>
    </View>
  );
};
...
const CenterOfInterestScreen = ({ navigation }: CentersOfInterestScreenProps) => {
  const [statefulState, setStatefulState] = useState<ViewState<CentersOfInterestInteractorResult>>(
    ViewState.Loading(),
  );
