...
export const useNewsScreen = (): {
  ...
  const navigation = useNavigation<NewsNavigatorScreenProps<"News">["navigation"]>();
  const [statefulState, setStatefulState] = useState<ViewState<PaginatedResult<Array<News>>>>(
    ViewState.Loading(),
  );
