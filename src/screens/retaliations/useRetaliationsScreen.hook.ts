...
export const useRetaliationsScreen = (): {
  ...
  const navigation = useNavigation<ActionsNavigatorScreenProps<"Retaliations">["navigation"]>();
  const [statefulState, setStatefulState] = useState<ViewState<Array<Retaliation>>>(
    ViewState.Loading(),
  );
