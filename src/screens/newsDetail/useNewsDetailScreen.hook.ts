...
export const useNewsDetailScreen = (
  ...
  const [statefulState, setStatefulState] = useState<ViewState<News>>(ViewState.Loading());
  ...
  useEffect(() => {
    const fetchNews = () => {
      NewsRepository.getInstance()
        ...
        .catch((error) => setStatefulState(ViewStateUtils.networkError(error, fetchNews)));
    };
    fetchNews();
  }, [newsId]);
